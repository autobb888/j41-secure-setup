...
function teardownChain(gatewayIp) {
  for (const bin of ['iptables', 'ip6tables']) {
    ...
    // Second pass: delete any INPUT ACCEPT for the proxy port WITHOUT -d, to
    // catch a stale rule that was added under a now-gone gateway IP. iptables -D
    // matches by exact spec, so a rule added WITH -d gatewayIp is NOT deleted
    // by a -D WITHOUT -d (and vice-versa) — both forms are needed to cover both
    // the current add (with -d) and any hypothetical stale rule (without -d or
    // with a different -d). ip6tables will simply fail to match and break early;
    // that is intentional (the ACCEPT rule is IPv4-only via iptables4).
    for (let i = 0; i < 10; i++) {
      try {
        execFileSync('sudo', [bin, '-D', 'INPUT', '-i', BRIDGE_IF, '-p', 'tcp', '--dport', String(EGRESS_PROXY_PORT), '-j', 'ACCEPT'], { stdio: 'pipe', timeout: 10000 });
      } catch { break; }
    }
    for (let i = 0; i < 10; i++) {
      try {
