...
/**
 * Check whether runsc is Docker's default runtime.
 * Prefer `docker info` (authoritative, works for any docker-group user, and
 * matches jailbox docker.ts) — reading /etc/docker/daemon.json directly fails
 * for a non-root user when the file is root-only, which made quickCheck report
 * bwrap mode even with gVisor active.
 * @returns {boolean}
 */
function isGvisorDefault() {
  try {
    ...
    if (rt) return rt === 'runsc';
  } catch { /* docker not reachable — fall back to the daemon.json file */ }
  try {
    const cfg = JSON.parse(fs.readFileSync('/etc/docker/daemon.json', 'utf8'));
...
function areSeccompProfilesDeployed() {
  ...
  const dir = platform === 'linux' ? '/etc/j41' : path.join(os.homedir(), '.j41');

  // /etc/j41 is 0711 (traversable, NOT listable), so readdirSync fails for a
  // non-root user — probe known profile filenames via existsSync instead, which
  // only needs directory-traverse (x), not read (r).
  const known = ['seccomp-jailbox.json', 'seccomp-agent.json'];
  try {
    return known.some((f) => fs.existsSync(path.join(dir, f)));
  } catch {
    return false;
  }
}
