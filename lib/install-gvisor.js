 ...
 *
 * @param {'kvm' | 'systrap'} platform gVisor platform. `kvm` is fastest but
 *   needs /dev/kvm; `systrap` needs NO KVM (works on cloud VMs without nested
 *   virt) and is gVisor's modern default. We pin it explicitly so the runtime
 *   doesn't silently fall back to a platform the host can't provide.
 */
function configureDaemonJson(platform) {
  ...
  cfg.runtimes.runsc = { path: 'runsc', runtimeArgs: [`--platform=${platform}`] };
 ...
 *
 * @param {{ distro: string, arch: string, hasKVM?: boolean }} options
 *   hasKVM selects the gVisor platform: `kvm` (fastest) when /dev/kvm exists,
 *   else `systrap` (no KVM required). gVisor runs fine without KVM — only the
 *   platform differs — so installation is NOT gated on KVM.
 * @returns {Promise<{ success: boolean, platform: string, steps: Array<{step: string, status: string, [key: string]: unknown}> }>}
 */
export async function installGvisor({ distro, arch, hasKVM = false }) {
  const steps = [];
  const gvisorArch = mapArch(arch);
  const platform = hasKVM ? 'kvm' : 'systrap';
  ...
  try {
    configureDaemonJson(platform);
    steps.push({ step: 'configure-docker', status: 'ok', platform });
  } catch (err) {
    steps.push({ step: 'configure-docker', status: 'error', error: err.message });
    return { success: false, platform, steps };
  }
  ...
  return { success: true, platform, steps };
}
