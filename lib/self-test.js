...
function testForkBomb(baseArgs) {
  // Audit 2026-06-02 L-SECURE-SETUP-ddos-1: add memory + cpu caps so a
  // misbehaving fork-bomb test that escapes PID limits cannot OOM the
  // operator's host while we're attempting to verify isolation.
  const argsWithPids = [...baseArgs, '--pids-limit 64', '--memory 256m', '--cpus 0.5'];
  const bomb = ':(){ :|:& };: ; sleep 2';
...
/**
 * no-direct-egress (dispatcher) -- assert that a container on j41-isolated
 * CANNOT reach external hosts directly. Under the proxy-only model, the sandbox
 * has zero direct external egress; all outbound traffic goes through the host
 * egress proxy at gatewayIp:EGRESS_PROXY_PORT. A direct HTTP fetch to an
 * external host must be BLOCKED by the J41_AGENT_OUT iptables chain.
 *
 * PASS: any non-zero exit (timeout, connection refused, network unreachable,
 *       DNS failure, or any other error) — the request was blocked as required.
 * FAIL: exit 0 — wget fetched the URL, meaning direct egress is NOT blocked
 *       and the proxy-only firewall is not enforced.
 *
 * Block-detection is intentionally broad: we do NOT pattern-match on error text.
 * Any failure whatsoever counts as blocked; only a clean exit 0 is a failure.
 */
function testNoDirectEgress(baseArgs) {
  let stdout = '';
  let stderr = '';
  let exitCode = 0;

  try {
    const cmd = `docker run ${baseArgs.join(' ')} ${IMAGE} wget -q -O- --timeout=5 https://api.junction41.io`;
    stdout = docker(cmd, 20000);
    exitCode = 0;
  } catch (err) {
    exitCode = err.status ?? 1;
    stdout = err.stdout ? err.stdout.toString() : '';
  ...
  if (exitCode === 0) {
    // wget exited 0 → it fetched content → direct egress is reachable → FAIL
    throw new Error(
      `Container on j41-isolated reached api.junction41.io directly — ` +
      `proxy-only firewall (J41_AGENT_OUT) is not enforced. ` +
      `Output: ${(stdout + stderr).slice(0, 300)}`,
    );
  }
  // Non-zero exit for any reason (timeout, refused, unreachable, dns-fail, etc.)
  // means the direct egress attempt was blocked → PASS
}
...
async function testCanaryInjection() {
  ...
  try {
    const sdk = await import('@junction41/sovagent-sdk');
  ...
  // Test 2b: Verify evasion resistance — inject zero-width spaces into canary
  const evadedCanary = fakeCanary.split('').join('\u200B'); // zero-width space between each char
  if (!checkForCanaryLeak(evadedCanary, fakeCanary)) {
    throw new Error('checkForCanaryLeak failed to detect zero-width-evaded canary — evasion resistance broken');
  }

  // Verify no false-positive on clean text
  ...
  const possiblePaths = await getDispatcherPaths();
  ...
  for (const p of possiblePaths) {
    if (fs.existsSync(p)) {
      ...
      if (!content.includes('_sdkCanaryCheck')) {
        throw new Error(`job-agent.js at ${p} is using simple string match instead of SDK evasion-resistant check — update @junction41/dispatcher`);
      }
      break;
    }
  }
  if (!found) {
    throw new Error('Could not locate @junction41/dispatcher job-agent.js — install globally to verify canary wiring');
  }
}

/** Resolve possible paths to the globally installed dispatcher's job-agent.js */
async function getDispatcherPaths() {
  const paths = [
  ...
  try {
    ...
    paths.push(path.join(globalPrefix, 'lib', 'node_modules', '@junction41', 'dispatcher', 'src', 'job-agent.js'));
  } catch {}
  return paths;
}
...
export async function selfTest(product) {
  ...
  if (product === 'dispatcher') {
    results.push(
      await runTest('no-direct-egress', () => testNoDirectEgress(baseArgs)),
