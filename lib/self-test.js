...
/**
 * container-escape — attempt to read /etc/shadow from inside the container.
 * Should fail with "Permission denied" or "No such file".
 */
function testContainerEscape(baseArgs) {
  const cmd = `docker run ${baseArgs.join(' ')} ${IMAGE} cat /etc/shadow`;
  assertContainerFails(cmd, /Permission denied|No such file|cannot open|nonexistent/i);
}
